...
from model.path_rank import PathRank
...
_grid_id_padding_idx = None

def init(geo_dict, trans_prob_mat, road_id2grid_id, road_id_padding_idx, grid_id_padding_idx):
    global _geo_dict, _trans_prob_mat, _road_id2grid_id, _road_id_padding_idx, _grid_id_padding_idx

    _geo_dict = geo_dict
    _trans_prob_mat = trans_prob_mat
    _road_id2grid_id = road_id2grid_id
    _road_id_padding_idx = road_id_padding_idx
    _grid_id_padding_idx = grid_id_padding_idx
...
def process_row(args):
    ...
    for i in range(len(path)):
        ...
        adj_grid_id_data[i] = np.pad(adj_grid_id_data[i], (0, pad_num), 'constant', constant_values=_grid_id_padding_idx)
...
class MyCollateFn:
    ...
    def __call__(self, items):
        ...
        for i, item in enumerate(items):
            ...
            batch["label"][i] = torch.as_tensor(item["label"], dtype=torch.float32)
...
if __name__ == '__main__':
    ...
    train_traj = pd.read_csv(f'./data/{args.dataset}/out/path_rank_train.csv')
    val_traj = pd.read_csv(f'./data/{args.dataset}/out/path_rank_val.csv')
    test_traj = pd.read_csv(f'./data/{args.dataset}/out/path_rank_test.csv')
    ...
    path_rank_config = dict_to_namespace({
        'hidden_dim': args.hidden_dim,
    })
    path_rank = PathRank(path_rank_config).to(device)

    os.makedirs(f'./output/{args.exp_name}/log/{args.dataset}/path_rank/seed_{args.seed}', exist_ok=True)
    logger.remove()
    logger.add(os.path.join(f'./output/{args.exp_name}/log/{args.dataset}/path_rank/seed_{args.seed}', f'{datetime.now().strftime("%Y-%m-%d-%H-%M-%S")}.log'), level='INFO', format='{time:YYYY-MM-DD HH:mm:ss} | {message}')

    logger.info(f'args: {args}')
    logger.info(f'core_config: {core_config}')

    os.makedirs(f'./output/{args.exp_name}/save/{args.dataset}/path_rank/seed_{args.seed}', exist_ok=True)

    optimizer = torch.optim.AdamW(list(core.parameters())+list(path_rank.parameters()), lr=args.lr, betas=eval(args.betas), weight_decay=args.weight_decay)
    ...
    for epoch_id in range(args.epoch):
        ...
        for batch in tqdm(train_dataloader, desc=f'[Training] epoch {epoch_id}'):
            ...
            pred = path_rank(traj_embedding)

            loss = F.mse_loss(pred, batch['label'])

            optimizer.zero_grad()
            loss.backward()
            torch.nn.utils.clip_grad_norm_(list(core.parameters())+list(path_rank.parameters()), max_norm=1.0)
            optimizer.step()
        ...
        for batch in tqdm(val_dataloader, desc=f'[Validating] epoch {epoch_id}'):
            ...
            mae_array.extend(torch.abs(pred - batch['label']).cpu().numpy().tolist())
            pred_array.extend(pred.cpu().numpy().tolist())

        mae = np.mean(mae_array)
        label_array = val_traj['label'].to_list()
        group_info = val_traj['group'].to_list()
        num_groups = np.max(group_info) + 1
        grouped_pred = [[] for _ in range(num_groups)]
        grouped_label = [[] for _ in range(num_groups)]
        for i in range(len(group_info)):
            grouped_pred[group_info[i]].append(pred_array[i])
            grouped_label[group_info[i]].append(label_array[i])

        kendall = np.mean([kendalltau(grouped_pred[i], grouped_label[i])[0] for i in range(num_groups) if (len(grouped_pred[i]) > 1 and not np.all(grouped_pred[i] == grouped_pred[i][0]))])
        spearman = np.mean([spearmanr(grouped_pred[i], grouped_label[i])[0] for i in range(num_groups) if (len(grouped_pred[i]) > 1 and not np.all(grouped_pred[i] == grouped_pred[i][0]))])

        logger.info(f'[Epoch {epoch_id + 1}] loss: {np.mean(loss_array)}, maxvio: {np.mean(maxvio_array)}, lr: {optimizer.param_groups[0]["lr"]}, MAE: {mae}, Kendall: {kendall}, Spearman: {spearman}')
        if kendall > best_kendall:
            best_kendall = kendall
            logger.info(f'New best Kendall: {kendall:.4f}. Saving model...')
            torch.save(core.state_dict(), f'./output/{args.exp_name}/save/{args.dataset}/path_rank/seed_{args.seed}/core_best.pth')
            torch.save(path_rank.state_dict(), f'./output/{args.exp_name}/save/{args.dataset}/path_rank/seed_{args.seed}/path_rank_best.pth')

    core.load_state_dict(torch.load(f'./output/{args.exp_name}/save/{args.dataset}/path_rank/seed_{args.seed}/core_best.pth', map_location=device, weights_only=True))
    path_rank.load_state_dict(torch.load(f'./output/{args.exp_name}/save/{args.dataset}/path_rank/seed_{args.seed}/path_rank_best.pth', map_location=device, weights_only=True))
    ...
    mae = np.mean(mae_array)
    label_array = test_traj['label'].to_list()
    group_info = test_traj['group'].to_list()
    num_groups = np.max(group_info) + 1
    grouped_pred = [[] for _ in range(num_groups)]
    grouped_label = [[] for _ in range(num_groups)]
    for i in range(len(group_info)):
    ...
    logger.info(f'[Testing] MAE: {mae}, Kendall: {kendall}, Spearman: {spearman}')
