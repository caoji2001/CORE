...
class TrajDataset(Dataset):
    def __init__(self, geo_dict, trans_prob_mat, road_id2grid_id, road_id_padding_idx, grid_id_padding_idx, traj):
        ...
        tasks = [(traj.at[i, 'path'], traj.at[i, 'timestamp']) for i in range(len(traj))]
...
if __name__ == '__main__':
    ...
    detoured_traj['path'] = detoured_traj['path'].apply(eval)
    detoured_traj['timestamp'] = detoured_traj['timestamp'].apply(eval)

    not_detoured_traj['path'] = not_detoured_traj['path'].apply(eval)
    not_detoured_traj['timestamp'] = not_detoured_traj['timestamp'].apply(eval)

    other_traj['path'] = other_traj['path'].apply(eval)
    other_traj['timestamp'] = other_traj['timestamp'].apply(eval)

    assert args.dataset in ['Beijing', 'Chengdu', 'Xian', 'Porto']
    if args.dataset in ['Beijing', 'Chengdu', 'Xian']:
        detoured_traj['timestamp'] = detoured_traj['timestamp'].apply(
            lambda lst: [datetime.fromtimestamp(t, ZoneInfo('Asia/Shanghai')) for t in lst]
        )
        not_detoured_traj['timestamp'] = not_detoured_traj['timestamp'].apply(
            lambda lst: [datetime.fromtimestamp(t, ZoneInfo('Asia/Shanghai')) for t in lst]
        )
        other_traj['timestamp'] = other_traj['timestamp'].apply(
            lambda lst: [datetime.fromtimestamp(t, ZoneInfo('Asia/Shanghai')) for t in lst]
        )
    else:
        detoured_traj['timestamp'] = detoured_traj['timestamp'].apply(
    ...
    detoured_dataset = TrajDataset(geo_dict, trans_prob_mat, road_id2grid_id, road_id_padding_idx, grid_id_padding_idx, detoured_traj)
    detoured_dataloader = DataLoader(
        detoured_dataset,
    ...
    not_detoured_dataset = TrajDataset(geo_dict, trans_prob_mat, road_id2grid_id, road_id_padding_idx, grid_id_padding_idx, not_detoured_traj)
    not_detoured_dataloader = DataLoader(
        not_detoured_dataset,
        batch_size=args.batch_size,
        shuffle=False,
        collate_fn=MyCollateFn(road_id_padding_idx, grid_id_padding_idx, weekday_padding_idx, time_of_day_padding_idx),
        num_workers=8,
    ...
    other_dataset = TrajDataset(geo_dict, trans_prob_mat, road_id2grid_id, road_id_padding_idx, grid_id_padding_idx, other_traj)
    ...
    os.makedirs(f'./output/{args.exp_name}/log/{args.dataset}/traj_sim/seed_{args.seed}', exist_ok=True)
    logger.remove()
    logger.add(os.path.join(f'./output/{args.exp_name}/log/{args.dataset}/traj_sim/seed_{args.seed}', f'{datetime.now().strftime("%Y-%m-%d-%H-%M-%S")}.log'), level='INFO', format='{time:YYYY-MM-DD HH:mm:ss} | {message}')

    logger.info(f'args: {args}')
    logger.info(f'core_config: {core_config}')

    detoured_traj_emb, not_detoured_traj_emb, other_traj_emb = [], [], []
    core.eval()

    for batch in tqdm(detoured_dataloader, desc=f'[Calculating detoured traj embeddings]'):
        for k in batch.keys():
        ...
        detoured_traj_emb.append(traj_embedding.cpu().numpy())

    for batch in tqdm(not_detoured_dataloader, desc=f'[Calculating not detoured traj embeddings]'):
        for k in batch.keys():
            batch[k] = batch[k].to(device, non_blocking=True)
        ...
        not_detoured_traj_emb.append(traj_embedding.cpu().numpy())

    for batch in tqdm(other_dataloader, desc=f'[Calculating other traj embeddings]'):
        for k in batch.keys():
            batch[k] = batch[k].to(device, non_blocking=True)
        ...
        other_traj_emb.append(traj_embedding.cpu().numpy())

    detoured_traj_emb = np.vstack(detoured_traj_emb)
    not_detoured_and_other_traj_emb = np.concatenate((np.vstack(not_detoured_traj_emb), np.vstack(other_traj_emb)), axis=0)

    distances = cdist(detoured_traj_emb, not_detoured_and_other_traj_emb, metric='cosine')

    predicted_indices = np.argmin(distances, axis=1)
    correct = predicted_indices == np.arange(5000)
    acc_at_1 = np.mean(correct)

    top5_indices = np.argsort(distances, axis=1)[:, :5]
    correct = np.any(top5_indices == np.arange(5000)[:, np.newaxis], axis=1)
    acc_at_5 = np.mean(correct)

    sorted_indices = np.argsort(distances, axis=1)
    ranks = np.where(sorted_indices == np.arange(5000)[:, np.newaxis])[1] + 1
    mr = np.mean(ranks)
    mrr = np.mean(1 / ranks)

    logger.info(f'[Testing] Acc@1: {acc_at_1}, Acc@5: {acc_at_5}, MR: {mr}, MRR: {mrr}')
