...
if __name__ == '__main__':
    ...
    train_traj['label'] = train_traj['timestamp'].apply(lambda x: (x[-1] - x[0]) / 60.0)

    val_traj['path'] = val_traj['path'].apply(eval)
    val_traj['timestamp'] = val_traj['timestamp'].apply(eval)
    val_traj['label'] = val_traj['timestamp'].apply(lambda x: (x[-1] - x[0]) / 60.0)

    test_traj['path'] = test_traj['path'].apply(eval)
    test_traj['timestamp'] = test_traj['timestamp'].apply(eval)
    test_traj['label'] = test_traj['timestamp'].apply(lambda x: (x[-1] - x[0]) / 60.0)
    ...
    if args.dataset in ['Beijing', 'Chengdu', 'Xian']:
        train_traj['timestamp'] = train_traj['timestamp'].apply(
            lambda lst: [datetime.fromtimestamp(lst[0], ZoneInfo('Asia/Shanghai')) for _ in range(len(lst))]
        )
        ...
        train_traj['timestamp'] = train_traj['timestamp'].apply(
            lambda lst: [datetime.fromtimestamp(lst[0], ZoneInfo('Europe/Lisbon')) for _ in range(len(lst))]
        )
        val_traj['timestamp'] = val_traj['timestamp'].apply(
            lambda lst: [datetime.fromtimestamp(lst[0], ZoneInfo('Europe/Lisbon')) for _ in range(len(lst))]
        )
    ...
    tte_config = dict_to_namespace({
        'hidden_dim': args.hidden_dim,
    })
    tte = TTE(tte_config).to(device)

    os.makedirs(f'./output/{args.exp_name}/log/{args.dataset}/tte/seed_{args.seed}', exist_ok=True)
    logger.remove()
    logger.add(os.path.join(f'./output/{args.exp_name}/log/{args.dataset}/tte/seed_{args.seed}', f'{datetime.now().strftime("%Y-%m-%d-%H-%M-%S")}.log'), level='INFO', format='{time:YYYY-MM-DD HH:mm:ss} | {message}')

    logger.info(f'args: {args}')
    logger.info(f'core_config: {core_config}')

    os.makedirs(f'./output/{args.exp_name}/save/{args.dataset}/tte/seed_{args.seed}', exist_ok=True)

    optimizer = torch.optim.AdamW(list(core.parameters())+list(tte.parameters()), lr=args.lr, betas=eval(args.betas), weight_decay=args.weight_decay)
    ...
    for epoch_id in range(args.epoch):
        ...
        for batch in tqdm(train_dataloader, desc=f'[Training] epoch {epoch_id}'):
            ...
            pred = tte(traj_embedding)

            loss = F.mse_loss(pred, batch['label'])

            optimizer.zero_grad()
            loss.backward()
            torch.nn.utils.clip_grad_norm_(list(core.parameters())+list(tte.parameters()), max_norm=1.0)
            optimizer.step()
        ...
        mae_array, rmse_array, mape_array = [], [], []
        for batch in tqdm(val_dataloader, desc=f'[Validating] epoch {epoch_id}'):
            ...
            rmse_array.extend(((pred - batch['label']) ** 2).cpu().numpy().tolist())
            mape_array.extend((torch.abs(pred - batch['label']) / batch["label"]).cpu().numpy().tolist())

        mae = np.mean(mae_array)
        rmse = np.sqrt(np.mean(rmse_array))
        mape = np.mean(mape_array)

        logger.info(f'[Epoch {epoch_id + 1}] loss: {np.mean(loss_array)}, maxvio: {np.mean(maxvio_array)}, lr: {optimizer.param_groups[0]["lr"]}, MAE: {mae}, RMSE: {rmse}, MAPE: {mape}')
        if mae < best_mae:
            best_mae = mae
            logger.info(f'New best MAE: {mae:.4f}. Saving model...')
            torch.save(core.state_dict(), f'./output/{args.exp_name}/save/{args.dataset}/tte/seed_{args.seed}/core_best.pth')
            torch.save(tte.state_dict(), f'./output/{args.exp_name}/save/{args.dataset}/tte/seed_{args.seed}/tte_best.pth')

    core.load_state_dict(torch.load(f'./output/{args.exp_name}/save/{args.dataset}/tte/seed_{args.seed}/core_best.pth', map_location=device, weights_only=True))
    tte.load_state_dict(torch.load(f'./output/{args.exp_name}/save/{args.dataset}/tte/seed_{args.seed}/tte_best.pth', map_location=device, weights_only=True))
    ...
    logger.info(f"[Testing] MAE: {mae}, RMSE: {rmse}, MAPE: {mape}")
